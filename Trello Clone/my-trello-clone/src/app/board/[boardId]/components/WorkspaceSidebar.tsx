...
import { InviteMemberModal } from '@/components/invite-member-modal'; // Hãy chắc chắn đường dẫn này đúng
...
export const WorkspaceSidebar = ({ workspaceName, activeBoardName }: WorkspaceSidebarProps) => {
    ...
    // 2. Thêm state để quản lý việc mở/đóng modal
    const [isInviteModalOpen, setIsInviteModalOpen] = useState(false);

    return (
        // Sử dụng Fragment <>...</> để bọc nhiều component
        <>
            <aside className="w-64 border-r border-gray-200 p-2 flex flex-col shrink-0 bg-purple-800 text-white">
                ...
                <nav className="flex-grow space-y-1 px-2">
                    ...
                    {/* 3. Chuyển Link thành Button và thêm onClick để mở modal */}
                    <button
                        onClick={() => setIsInviteModalOpen(true)}
                        className="w-full flex items-center gap-3 p-2 rounded-md text-sm font-semibold text-gray-200 hover:bg-purple-700/50 text-left"
                    >
            ...
            {/* 4. Render component modal ở đây. Nó sẽ chỉ hiển thị khi isInviteModalOpen là true */}
            <InviteMemberModal
